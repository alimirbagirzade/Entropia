...
## QA kalite kapıları — coverage + dependency audit + a11y (PR #389)

**Sorun (ampirik):** `grep -niE 'cov|a11y|axe|npm audit|pip audit' .github/workflows/*.yml` → **0 hit**.
CI her iki süiti koşuyor ve coverage basıyordu, ama **hiçbiri üzerinden bir şey kırılamıyordu**:

- `backend/pyproject.toml` `addopts`'ta `--cov=entropia` vardı, `--cov-fail-under` **yoktu**; `[tool.coverage]` bölümü hiç yoktu.
- Frontend'de coverage provider **kurulu bile değildi** (`@vitest/coverage-v8` yok). Not: `frontend/vitest.config.ts` **yok** — vitest yapılandırması `frontend/vite.config.ts` `test:` bloğunda.
- Hiçbir workflow dependency audit koşmuyordu.
- R2-14'te yazılan `frontend/e2e/specs/13-a11y-scan.spec.ts` ve `npm run a11y` script'i vardı ama **hiçbir job'a bağlanmamıştı** — a11y regresyonu sessizce inebilirdi.

**Ne indi:**

| Alan | Değişiklik |
|---|---|
| Backend coverage | `[tool.coverage.run]` + `[tool.coverage.report]` (`omit = ["*/__main__.py"]` — üç `python -m` süreç giriş noktası; `exclude_lines`: `pragma: no cover`, `if TYPE_CHECKING:`, `@overload`, `@abstractmethod`, `^\s*\.\.\.$`, `raise NotImplementedError`, `if __name__ == .__main__.:`), `precision = 1` · `addopts` → `--cov-fail-under=80` |
| Frontend coverage | `@vitest/coverage-v8@^4.1.10` · `coverage` script'i (`vitest run --coverage --testTimeout=20000`) · `vite.config.ts` `test.coverage`: v8 provider, `include: src/**`, `exclude`: `src/test/**`, `main.tsx`, `*.d.ts`, `*.generated.ts`, `reportOnFailure: true`, `thresholds: {lines 80, statements 78, functions 70, branches 68}` |
| Dependency audit | `ci.yml` backend → `uv run --with pip-audit pip-audit` (bloklayıcı) · frontend → `node scripts/npm-audit-gate.mjs frontend frontend/e2e` |
| A11Y | `e2e.yml`'e yeni `a11y` job'ı (kendi runner'ı, kendi Compose projesi; aynı seed; `npm run a11y`; `a11y-report` artefaktı `if: always()`) |
| Doküman | `README.md` §Verifying changes + `CONTRIBUTING.md` adım 4/7 kapıları anlatır · `.gitignore` → `frontend/coverage/` |

**`scripts/npm-audit-gate.mjs` — neden çıplak `npm audit --audit-level=high` değil:**
Frontend'de üç high advisory var ve **hiçbiri kırıcı olmayan yolla düzelmiyor** —
`GHSA-3jxr-9vmj-r5cp` + `GHSA-mh99-v99m-4gvg` (`brace-expansion`, dev-only zincir: typescript-eslint → minimatch;
npm'in düzeltme yolu `eslint@10` major'ı) ve `GHSA-qwww-vcr4-c8h2` (`react-router` RSC-mode CSRF; uygulama Vite SPA'sı
`BrowserRouter` üzerinde, RSC modu hiç açılmıyor; `react-router-dom@7.18.1` `react-router@7.18.1`'i **tam** pinliyor,
yamalı hat `react-router@8.2.1+` yani `react-router-dom`'u kaldıran v8 migration'ı).
`npm audit fix` denendi ve durumu **kötüleştirdi** (3 high → 7 high) — geri alındı.
Ayrıca `.github/dependabot.yml:31-37` frontend major'larını bilinçle bastırıyor ("cannot be merged piecemeal").
Çıplak audit her PR'da kırmızı olurdu; `continue-on-error` ise **hiç kırılamayan** bir adım bırakırdı.
Bu yüzden kapı, a11y taramasındaki `ACCEPTED_SERIOUS_RULES` deyimini aynalıyor: bu üç id **gerekçesiyle dondurulmuş**,
listede olmayan her high/critical build'i kırıyor. Bayat girdi için de uyarı basıyor.

**Dondurulmuş a11y sınırı — DEĞİŞMEDİ:** `ACCEPTED_SERIOUS_RULES` = `["color-contrast"]`, PO'nun D-8 kararının
bıraktığı hâliyle birebir. `link-in-text-block` düzeltilip (paragraf içi linkler `p a` ile altı çizili) listeden
**çıkarılmıştı**; geri eklemek D-8'i sessizce geri alırdı. Listeyi genişletmek PO kararıdır
(`v18_final_acceptance.md` §A11Y), CI kolaylığı değil.

**Kapıların gerçekten tetiklendiğinin kanıtı (her kapı iki yönde):**

```
backend  tam süit   TOTAL 22261 2220  90%                          → 80 eşiğini geçer
backend  alt küme   FAIL Required test coverage of 80% not reached. Total coverage: 0.27%
frontend geçici 80/80/80/75 eşikleriyle:
         ERROR: Coverage for functions (73.38%) does not meet global threshold (80%)
         ERROR: Coverage for branches (70.68%) does not meet global threshold (75%)
frontend kalibre 80/78/70/68 ile: eşik hatası YOK
         Lines 84.1 · Statements 81.68 · Functions 74.4 · Branches 71.58
npm gate un-freeze → FAIL — 1 unrecorded high/critical advisory(ies).   exit=1
npm gate restore   → OK — no unrecorded high/critical advisories.       exit=0
pip-audit          → No known vulnerabilities found
```

**Honest boundary'ler:**

- **Frontend eşikleri bir ALT SINIRDAN kalibre edildi.** Ölçüm makinesinde 6+ eşzamanlı agent oturumu
  koşuyordu (load ~26); bu, Testing Library'nin **1000 ms**'lik `findBy*` bekleme sınırını aşırıp izole
  koşuda geçen bir avuç testi düşürüyor. Tam yeşil olmayan bir koşudan ölçülen coverage **olduğundan düşüktür** —
  temiz bir koşu en az bu kadar ölçer. Eşikler bu tabanın birkaç puan altına konuldu ki yanlış alarm vermesin;
  **temiz bir koşu gerçek sayıları verdiğinde yükseltilmeli**.
- **Backend süiti kendi veritabanında yeşil.** Tam koşuda tek bir contract testi düştü, izole koşuda 3/3 geçiyor.
  Daha önceki kitlesel `F`/`E` yığını `tests/integration/db.py:40-50`'e izlendi: süit `TEST_DATABASE_URL` yoksa
  `DATABASE_URL`'den **`<db>_test`** türetiyor, dolayısıyla paralel worktree oturumları ortak `entropia_test`'te
  çarpışıyor. Bu PR o davranışa dokunmuyor — ölçümler `entropia_qagates_test` üzerinde yapıldı.
- **`uv pip audit` diye bir alt komut yok** (uv 0.11.26). Backend adımı gerçek `pip-audit`'i `uv run --with` ile koşar.
- `a11y` job'ı henüz bir runner'da koşmadı; PR #389 onun ilk koşusu.
- Coverage eşikleri **global**; per-diff (yeni kod) coverage zorlaması yok — ayrı bir iş.
