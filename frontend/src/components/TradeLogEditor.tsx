...
import { TradeLogConfigEditor } from "@/components/TradeLogConfigForm";
import { useTradeLogConfigEditorState } from "@/lib/tradeLogForm";
import {
...
// Signal editor (UI-04): a 2-column inline panel (TRADE LOG SOURCE / BULK TRADE
...
//
// R2-01a — this editor body was lifted VERBATIM out of pages/TradeLog.tsx so it
// can be mounted twice: as the /trade-log page (mode="page", the URL carries
// ?job= / ?root=) and, from R2-01b onward, inline on a Mainboard row
...
export type TradeLogEditorProps = {
  mode: "page" | "inline";
  initialRoot?: string;
...
// Trade Log editor (Stage 3d, doc 05 §8–§10). URL modes in page mode: ?job=
// (durable import handle, CR-09) and ?root= (work-object detail + revision
// composer). Pin ("Use This Revision") and delete are Mainboard operations
// (CR-01, TL-01).
export function TradeLogEditor({ mode, initialRoot, onSaved, onClose }: TradeLogEditorProps) {
...
// source asset id is SYSTEM-CARRIED from the file upload on the right (R2-04 /
// GAP item 3 fix #3 — never a user-editable technical field); instrument and
...
function CreatePanel({
  ...
  const editor = useTradeLogConfigEditorState(template);
  ...
  return (
    <>
      <section className="card" style={{ marginTop: 18 }} aria-labelledby="tl-create-h">
        ...
        <p className="cp-note">
          Requires a succeeded, non-empty record batch — the source binding is carried from the
          report automatically. Save is never a Ready PASS; with “attach” on (Save &amp; Add) the
...
function RevisionEditor({
  ...
  const editor = useTradeLogConfigEditorState(seedPayload);
