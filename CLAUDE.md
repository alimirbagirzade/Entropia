...
- **Next:** **post-V1 (continued)** — **NO teed-up (confirmed) open work remains.** Latest: capability
  READ surfaces — **output history (PR #143)** + **lifecycle-transition history (PR #144)** → **doc-22
  Future Dev capability system END-TO-END COMPLETE** (registry list/detail + Admin transition [OCC + 7
  gates + audit/outbox + activation event] + 2 operational POSTs [ACTIVE-only] + owner-scoped output
  history #143 + transition history #144). All route surfaces bound; TIER 2 page map 24/24; TIER 3
  candidates all closed (data-queue redelivery #129/#131, SSE reconnect #133, tool-call status shadowing
  #135, audit indexes #139/#141). **On session start: ASK the user for direction** — candidates (none
  confirmed): (a) minor backend cleanup/consistency (no migration); (b) a new user-brought feature; (c) an
  orphan/dead-code scan (any other repo function/endpoint/query left unbound — #144 closed exactly such an
  orphan; template: a written-but-unreadable append log → oldest-first projection + any-authenticated GET,
  write path stays gated). OUT OF SCOPE: retention auto-purge (doc 20 §16), LLM generation (Future-Dev),
  Graphic View renderer (doc 22 §1/§16/§17 "V18 static placeholder retained"). The **3 priority tiers**
  below are the historical record (all landed):
