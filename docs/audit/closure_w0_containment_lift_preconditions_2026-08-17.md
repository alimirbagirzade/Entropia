...
| 17 | OD-2 mark policy + `MARK_STALENESS_POLICY` flip | E6 | ❌ | `provenance.py::MARK_STALENESS_POLICY` = `"undefined_pending_od2"`. **2026-08-28 ÖLÇÜLDÜ (ADIM 129): BU SATIR KOD, VE `C9`-BAĞLI KOD.** ADR §13.1 OD-2 **(a)**'yı 2026-08-05'te KARARA BAĞLADI (carry-forward + bildirilmiş `stale_after` sınırı + diagnostic sayaç) — eksik olan **karar değil UYGULAMADIR** ve yokluğu *kasıtlıdır*: `clock.py::ItemTickView.staleness_ms` boşluğu **ÖLÇER, sınır UYGULAMAZ** ve `portfolio_ledger.py` ile birlikte ikisi de sınırı *"ADIM 20'nin tanıtacağı"* diye adlandırır. **Literali politikayı yazmadan çevirmek, `manifest.py`'nin kendi yorumunun yasakladığı yalandır.** Ayrıca **A16'dan sonra bu literal `execution_content` İÇİNDEDİR ve ARTIK HER SEVK EDİLEN RUN ONU TAŞIR** (`manifest.py::_portfolio_policy`, ADIM 126; *"engine-wide, so an independent single-Strategy run carries them too"*) → çevirmek **her `execution_key`'i kaydırır**, yani `C9`'un lift bump'ıyla aynı namespace kaymasıdır ve ADIM 126'nın dersi gereği ikinci kez ayrı harcanamaz. **Bu slice'ta UYGULANMADI** — ürün sahibi 2026-08-28'de `C` dedi (çözme, `C9`'a devret). **Satır KIRMIZI KALIR.** |
| 18 | `CONTENTION_SELECTION_STATUS` flip | E6 | ❌ | `arbitration.py::CONTENTION_SELECTION_STATUS` = `"recommended_pending_approval"`. **2026-08-28 ÖLÇÜLDÜ (ADIM 129): ADIN İMA ETTİĞİ ONAY BEKLEMİYOR — O ONAY ZATEN ALINDI.** ADR §13.1 (2026-08-05): *"Approval of this ADR resolves every open decision above to its stated recommendation"*; ADR **Accepted** (§16); OD-3 → **(a)**, *"Already the shipped behaviour"*. Sabitin **kendi docstring'i** bunu söylüyor: *"it describes the LABEL's state, not the decision's"*. Yani **17'nin aksine burada yazılacak kod YOK** — `provenance.py`'nin *"the same shape"* cümlesi bu eksende **yanlıştır**. **ADR'nin flip için gösterdiği TEK gerekçe ise ÇÜRÜDÜ:** §13.1 *"published through `build_portfolio_manifest`, **which nothing calls yet**"* diyor; bugün o fonksiyonun **üretim çağıranı VAR** (`portfolio_projection.py::project_portfolio_run` → worker `jobs/backtest_engine.py`). **YİNE DE ÇEVİRİLMEDİ, üç ölçülmüş sebeple:** (1) §13.1 flip'i açıkça *"ADIM 20 owns both flips"* diyor ve ADIM 20 = `C9`; (2) **hiçbir belge flip sonrası DEĞERİ adlandırmıyor** (tüm ağaç tarandı) → varsayılan seçmek ADIM 119'un yasakladığı şeydir; (3) `capability.py` REMOVAL CONDITION #4 arbitrajın **"symmetric"** olmasını ister, oysa ADR §13'ün kendi sözlüğünde *"fully symmetric, order-free"* = **(b)**, yani §13.1'in **almadığı** seçenek — ve `pin_order_admission` kıtlıkta düşük pin'i kayırır (**üç bağımsız ölçüm aynı fikirde**: C9 verdict §1 md. 4 · ADIM 128'in A4 çekişme ölçümü · ADR §13'ün kendi (b) tarifi). **Bu çelişkiyi çözmek ADJUDICATION'dır ve `capability.py` `C9`'undur** (ADIM 42 kuralı) → **ÜRÜN SAHİBİNE SORULDU (2026-08-28) ve cevap `C` OLDU: çelişki ŞİMDİ ÇÖZÜLMEYECEK, `C9`'a ADIYLA devredilecek.** Bu bir adjudication DEĞİL, adjudication'ı reddetmedir: ne `capability.py` #4 ne ADR §13.1 yeniden yazıldı, `CONTENTION_SELECTION_STATUS` **ELLENMEDİ** (negatif kontrol NC-3 bunu kanıtlar: etiketi çevirmek yalnız `test_the_od3_selection_rule_is_labelled_as_pending_approval`'ı kırmızıya çevirir, yani etiket bu slice'ın dokunmadığı, önceden var olan bir testin kapsamındadır). **Satır KIRMIZI KALIR ve `C9` artık SESSİZ değil ADLANDIRILMIŞ bir blocker devralır.** |
...
| 20 | GH **#544** (NET semantiği) kapalı | **insan/ürün** | ❌ **AÇIK** | `state: open`, `state_reason: reopened`. **2026-08-27: kararın KODU indi** — Karar 1'in `B` yarısı (`0044_drop_net_conflict_policy`: `NET` enum'dan düştü + kolon CHECK'i eklendi, `B3` halt guard'ı ile). Ön koşulun istediği şey issue'nun **kapanmasıdır** ve kapatma `human-only`; kalan tek eylem odur. **2026-08-27: O EYLEM GERÇEKLEŞTİ — #544 KAPANDI** (`13:05:47Z`, `state: closed`, `state_reason: completed`, kapatan `alimirbagirzade`; kapanış gerekçesi issue'da: `#issuecomment-5441715204`). **ADIM 90'ın üç ölçümü de aynı fikirde:** issue durumu · issue'da **yazılı** karar · imza kutusu (dördün dördü imzalı). İmzanın içine gömülü *"#544, `B` sevk edilene kadar KAPATILMAZ"* koşulu (`closure_g14_…md` §Karar 1/2/4) `B`'nin **#859** ile sevkiyle **tahliye edildi**. **KOLON BİLEREK `❌` BIRAKILDI** — #861'in kuralı: bu sütun `0f0651d`'ye aittir ve **donmuştur**, sonradan eklenen tarihli notlar onu değiştirmez (satır 13/14/15/16 ile aynı desen); taze sayım `PROJECT_HISTORY.md`'nin en son ADIM kaydındadır. **Yan ölçüm — küçük drift:** yukarıdaki *"kapatma `human-only`"* nitelemesi **yanlıştı**; #544'ün etiketleri `blocks-adim-19` + `product-decision`, `human-only` ise **#514**'ün etiketidir (ölçüldü: #514 onu taşır ve AÇIK). Kısıt yine de gerçekti. |
| 21 | GH **#559** (DST kuralı) kapalı | **insan/ürün** | ❌ **AÇIK** | `state: open`, `state_reason: reopened`, `blocks-mixed-zone-axis` |
| 22 | A15 bump + A16 manifest + A19 + A22 | E6 (`C9`) | ❌ | `ENGINE_VERSION` değişmedi; manifest dört policy alanını taşımıyor (`test_oracle_portfolio_containment_gate.py:246`). **2026-08-27: A16'NIN KODU İNDİ** (ADIM 126, `C7`) — manifest artık dört policy alanını **taşıyor** (`manifest.py::_portfolio_policy`, `execution_content` içinde) ve admission çözülmüş sleeve tutarlarını + FX ref'lerini pinliyor (`readiness_check.py::_resolve_allocation`; ikisi de zaten hesaplanıp **atılıyordu**). Adı geçen tripwire **ters çevrildi, silinmedi**. **`ENGINE_VERSION` DE BUMP EDİLDİ AMA BU A15'İ KARŞILAMAZ — kasıtlı ve imzalı** (ürün sahibi, karşı kanıt sunulduktan sonra yeniden onayladı): bump A16'nın **kayıt** değişikliği için harcandı, hiçbir davranış değişmedi (50 golden'ın 49'u bayt bayt aynı; oynayan tek digest `contract.execution_key`). A15'in koruduğu kusur — contained-era bir Result'ın unified-clock bir re-RUN için idempotent yeniden kullanımı — ancak **lift commit'inin kendi bump'ı** ile kapanır, ve `C9`'un bunu yapması artık hatırlanmıyor, **zorlanıyor**: `test_lifting_containment_requires_a_second_engine_version_bump` (dört köşeli predicate; NC-5'te ölçüldü — bayrak `active_v1`'e çevrilince kırmızı). **A19 + A22 EL DEĞMEDİ.** Satır bu yüzden **❌ KALIR**; kolon `0f0651d`'ye ait ve donmuştur. |
