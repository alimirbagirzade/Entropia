...
The O-03 sweep removed three dead definitions (``PrecheckAlreadyRunning``,
``DeletePolicyBlocked``, ``PublicationState.REMOVED``). Five never-raised error
classes remain as recorded debt in ``KNOWN_UNRAISED``; adding a sixth fails here.
"""

import re
from pathlib import Path

from entropia.domain.manual.enums import PublicationState
...
def test_publication_state_has_no_unassignable_member() -> None:
    """``removed`` claimed purge-time redaction that doc 21 never defines (O-03)."""
    assert [member.value for member in PublicationState] == ["draft", "published", "superseded"]
