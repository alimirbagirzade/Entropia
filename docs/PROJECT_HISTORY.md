...
## O-21 — Agent SSE sequence replay (PR #430, merged)
...
### Kayıt notu — bu bölüm bir kez kaybolup geri kondu (2026-07-28)

Kapanış dokümanları PR #439 ile main'e girdi (merge `c465466`, 4 dosyaya 286 satır). Ardından
#443 / #445 / #446 — hepsi #439'dan **önceki** main'den kesilmiş branch'ler — merge edilirken
`CLAUDE.md`, `docs/PROJECT_HISTORY.md` ve `docs/STAGE2_HANDOFF.md` bayat kopyalarıyla toptan
ezildi ve O-21 kaydı üç dosyadan da düştü. `docs/O21_LANDED_KICKOFF.md` yeni dosya olduğu için
çakışmadı ve hayatta kaldı. Aynı kusurun daha önceki örneği: `c30093b docs(o14): restore the
handoff, history and CLAUDE.md entries two merges dropped`.

**Yeniden doğrulama (restore anında, HEAD `72aaa6c`):** kod bozulmamış —
`uv run pytest -k "sse_replay" -q --no-cov` → **7 passed**;
`npx vitest run --no-file-parallelism -t "reconnect"` → **5 passed** (suite toplamı artık 630;
kapanış anındaki 614 rakamı O-21 öncesi sayıma göreydi). `Last-Event-ID` araması artık boş değil:
`backend/src` + `frontend/src` genelinde 21 isabet.

**Tekrarı önleme (kayıtlı ders):** bu üç dosya append-only ve çok-branch'li — bir slice'ı
kapatmadan önce `git fetch && git rebase origin/main` yap, merge'de bu dosyalarda **theirs/ours
toptan seçimi kullanma**, iki tarafın bölümlerini birlikte tut.
