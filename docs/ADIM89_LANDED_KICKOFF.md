...
# ADIM 89 LANDED — kabul borcu batch 15 (doc 02 backend): dört kriter kapandı · sıradaki slice için kickoff

> Tam kayıt: `docs/PROJECT_HISTORY.md` §ADIM 89. Bu belge **devam noktasıdır**, kayıt değil.

## Neredeyiz

- Taban main **`ee5ab38`**. **Ürün kodu değişmedi** (`backend/src` altında sıfır satır);
...
- **Tavanlar İNDİ: `partial` 83 → 79, `debt_class.B` 51 → 47**; açık borç **90 → 86**
  (A=1 · B=47 · C=6 · D=32). Clause `covered` 1034 → 1039, `uncovered` 95 → 91.
...
- `backend/tests/integration/test_strategy_integration.py` — dosyanın sonundaki
  **"Acceptance batch 15"** bölümü. Dört desen taşır:
...
5. **NUMARA VE PARTİ ETİKETİ AYNI ANDA ÇALINIR — bu hafta ÜÇÜNCÜ kez.** Bu slice yazılırken
   main'in son kaydı ADIM 88'di ve 89 boştu; ama **#802 `stage-90`**, **#803 `stage-91` +
   *"batch 15"*** adıyla **açıktı**. Kriterler ayrık (o doc 17 backend, bu doc 02 backend),
   **etiket değil.** Kapanışta `grep '^## ADIM'` **ve** `grep -o 'batch 1[0-9]'` koş, ikisini
   de commit'ten hemen önce doğrula. **Merge edilmiş ad kazanır.**
...
ORTAM: bu container'da Postgres 16 ikilileri KURULU. Integration suite'ini "koşamıyorum"
  diye atlama — cluster'ı kaldır (komutlar: docs/ADIM89_LANDED_KICKOFF.md §çapalar) ve KOŞ.
