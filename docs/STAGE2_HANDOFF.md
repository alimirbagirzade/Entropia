...
## I-17 — Acceptance test ID izlenebilirliği (PR #416)

**Ne landed.** `docs/spec` kabul tablolarındaki ID'lerin çoğu suite'ten **alıntılanamıyordu**:
bir kabul satırından onu karşılayan teste gitmenin yolu yoktu. İki aşama.

**(1) Etiketleme — davranış DEĞİŞMEDİ.** Doc 02/03/04/05/07/10'da **74 ID** artık karşıladığı
satırı adıyla anıyor (repoda zaten olan desen: `tests/unit/test_source_scan.py` → "PC-05/PC-06").
Sadece docstring/yorum. Kapsam içi izlenebilirlik **34/130 → 108/130**; tüm spec **88/215 → 162/215**.

**Sayım kuralı — yalnız TEST dosyaları sayılır.** Etiketli görünen bazı ID'ler aslında yalnız
production kaynağında anılıyordu (`lib/tradeLog.ts`, `pages/OutsourceSignal.tsx`); bu implementasyonu
belgeler, **test kanıtlamaz**. Bu yüzden `AOS-02`, `TL-01/14/15` etiketsiz sayıldı.

**Brief'in üç sayım düzeltmesi.** `PC-10` **zaten etiketliydi** (doc 07 = 4 etiketli / 18 etiketsiz,
3/19 değil). `TS-01…21 = 0`, doc 02'nin 16 `AT` ID'si ve `RF-06/12/13/15/16/18` **birebir doğrulandı**.

**(2) Gerçekten kapsanmayan 4 test yazıldı.** TL-04 (dosyasız Ready save → `import_binding` issue,
komutun `SOURCE_FILE_REQUIRED`'a çevirdiği girdi) · RF-12 (Family'siz Strategy Ready Check'i geçemez,
RUN kilitli kalır) · RF-16 (client-manipüle delete yine authenticated-actor + expected-version +
lifecycle kapılarına takılır, Trash entry yazılmaz) · AT-25 (her render edilen ⓘ katalog içeriğine
çözülür; panel açmak hiçbir form değerini değiştirmez).

**Doc 06/08/09 — ID sütunu YOK.** Spec dosyaları kanonik, **değiştirilmedi**. Bunun yerine
`docs/audit/acceptance_id_traceability.md` satır sırasına göre **audit-local** `CP-`/`PL-`/`ESP-`
ID'leri atıyor ve 57 satırın tamamını karşılayan testlere eşliyor.
`docs/audit/acceptance_id_scan.py` her sayıyı yeniden üretir.

**İki bulgu — sessizce halledilmedi.**
1. **KUSUR:** `StrategyConfig.rationale_family_id` `min_length` taşımıyor → `rationale_family_id: ""`
   parse ediliyor ve **READY** veriyor. RF-12 yalnız **eksik anahtara** karşı tutuyor; manipüle
   client Family'siz RUN'a ulaşabilir. Bu slice **düzeltmedi** (domain değişikliği, kapsam dışı);
   testin yanına ve audit §E.1'e sabitlendi. Takip: `fix/rf12-blank-rationale-family-blocks-ready`.
2. **Brief uyuşmazlığı:** AT-25 brief'te "Agent private root edit reddi" diye tarif edilmiş; doc 02
   §12 AT-25 aslında **"Info content"**. Spec'inki uygulandı. Tarif edilen davranış **AT-22**
   (zaten etiketli); Agent'a özel test istendiyse o **AT-21**, hâlâ açık.

**Doğrulama.** backend ruff + format + mypy temiz (356 dosya) · 1426 unit/contract geçti ·
frontend 60 dosya / 612 test geçti · **migration YOK, ENGINE_VERSION değişmedi.**
...
RF-15/ESP-05 · AOS-12 (`KIND_REVISION_MISMATCH` implementasyonu yok) · AT-24 · PC-14/19/22.
