...
## K-01 — Deklare timezone ingest'te uygulanıyor, sessiz UTC fallback kaldırıldı (PR #386, merge `eedc7f1`)

### Bulgu (ampirik doğrulandı)

`TimezoneSpec.zone` / `ResearchTimezoneSpec.zone` **ingest yolunda hiç çağrılmıyordu**. Tek iki
çağrı noktası dönüşü savunmacı biçimde atıyordu (`_ = timezone_spec.zone ...`,
`commands/market_data.py:439`, `commands/research_data.py:405`). `jobs/market_data.py::_parse_raw_bytes`
yalnız kolon eşliyordu; `validation_rules.parse_timestamp` her naive damgayı **koşulsuz UTC**
okuyordu (`:162, :182, :190`), `backtest/funding.py::parse_utc` de aynı (`:94-95`).

Sonuç: `America/New_York` deklare eden bir dataset gerçek anına **4 saat uzakta** saklanıyor,
engine tarafından öyle replay ediliyor ve yine de **auto-verify** olup `VERIFIED → APPROVED` ile
para-boyutlama motoruna giriyordu. İhlal edilen: doc 11 §4.3 / §7.3 adım 5 / §9.1 ("no silent UTC
fallback for ambiguous dates"), doc 12 §8.4 kural 1.

### Düzeltme

- **`source_zone` her timestamp okuyucusunda zorunlu keyword-only** — hiçbir çağrı noktası UTC'yi
  ihmal yoluyla miras alamaz: `parse_timestamp`, `evaluate_cross_row`, `funding.parse_utc`,
  `build_funding_schedule`, `quality_rules.evaluate_quality`.
- Yeni `resolve_timestamp() -> TimestampParse(moment, timezone_unresolved)` — *çöp* hücreyi
  *parse edilebilir ama yerelleştirilemez naive* hücreden ayırır; ikisi farklı bulgu üretir.
- Zone, **her iki durable job'da load ile validate arasında** uygulanır
  (`normalize_source_timestamps` / `normalize_event_timestamps`), böylece enjekte edilen test
  seam'leri dahil TÜM yükleme yolları normalize edilir.
- `revision_source_zone(revision)` persistence→domain adaptörü: `utc` → UTC, `custom` → saklı IANA,
  `exchange` → `None`. **Asla raise etmez** — IANA veritabanında bulunmayan saklı bir identifier
  worker'ı çökertmek yerine `None`'a düşer.
- Çözülemeyen naive hücre **aynen bırakılır** (normalizer asla uydurmaz) ve BLOCKING_FAIL üretir:
  yeni kanonik kodlar `MARKET_DATA_TIMEZONE_UNRESOLVED` (`shared.errors.MarketDataTimezoneUnresolved`,
  422 + `market_validation_issue.rule_code`) ve `RESEARCH_DATA_TIMEZONE_UNRESOLVED`
  (`ResearchDataTimezoneUnresolved`, 422 + `research_validation_issue.check_id`). Revision
  `NEEDS_REVIEW`'e düşer, auto-verify imkânsız.
- Source timezone **metadata'sı korunur** (`timezone_mode`/`timezone_iana`,
  `source_timezone_mode`/`source_timezone_iana` revision'da kalır) — doc 12 §8.4 kural 1.
- Engine'in 5 `parse_utc` çağrısı açıkça `source_zone=None` geçer: replay edilen veri ingest'te
  UTC-normalize edilmiştir, oradaki naive değer tahmin değil dürüst bir "yerleştirilemez" `None`'dır
  ve tüm çağrı noktaları `None`'ı zaten fail-closed işler.

### Geriye dönük: migration DEĞİL, audit

Migration **bilinçli olarak yazılmadı.** Bir revision'ın gerçekten yanlış olup olmadığı DB'de değil
**ham byte'larda**: offset taşıyan kaynaklar eski kodda da doğru parse ediliyordu; yalnız naive
kaynaklar kaydı. Bu bilgi object storage'da olduğundan toplu yeniden yazım **doğru veriyi bozardı**.

Yerine: `application/queries/timezone_audit.py` (read-only, **integration-test'li**) +
`scripts/audit_timezone_normalization.py` CLI. Tüketilebilir durumları
(`verified`/`approved`/`deprecated`/`approval_revoked`) tarar, `exchange` veya UTC olmayan `custom`
IANA deklarasyonlarını flagler. Lokal dev DB raporu: **0 / 0 at-risk**. Sıfırın "sorgu bozuk" değil
"risk altında veri yok" demek olduğunu iki integration testi kanıtlar (`custom/America/New_York` +
`exchange` flaglenir; `utc` + `draft` flaglenmez). Remediation: flaglenen revision'da **Analyze'ı
yeniden koş**.

### Testler

- **Unit:** `America/New_York` (DST sınırının **iki yanı** — EDT ve EST), `Asia/Istanbul`, `UTC`,
  naive-IANA-yok; ayrıca offset taşıyan değerin zone'u yok sayması, epoch'un mutlaklığı, çıplak
  `date` yerelleştirmesi ("ambiguous dates", doc 11 §9.1), çöp ile timezone-unresolved ayrımı.
- **Integration** (`tests/integration/test_ingest_timezone_normalization.py`, gerçek Postgres):
  gerçek zincir `CSV bytes → deterministik schema map → normalize → validate → processed Parquet`,
  parquet **geri okunup** doğrulanır: `America/New_York` `09:30` → `13:30+00:00` ·
  `Asia/Istanbul` `09:30` → `06:30+00:00` · `utc` değişmez · `exchange` aynen kalır →
  `BLOCKING_FAIL` + `NEEDS_REVIEW`. Research ingest native parquet'e karşı aynı biçimde kapsanır.

### Verify

`ruff` · `ruff format --check` · `mypy src` (348 dosya) temiz · tam backend suite **2014 passed /
0 failed** (izole DB) · **migration YOK** (alembic head `0035_portfolio_rules` değişmedi) ·
**`ENGINE_VERSION` değişmedi** — düzeltme upstream'de, ingest'in ne sakladığında.

### Dürüst sınırlar

- Audit revision'ları **risk altında** olarak raporlar, kesin-yanlış olarak değil; kesinleştirmek
  her ham asset'i object storage'dan okumayı gerektirir — bu slice bunu yapmaz.
- `exchange` modu **yapısı gereği çözümsüz kalır**: `TimezoneSpec.__post_init__` `custom` dışında
  IANA taşımayı yasaklar ve exchange→zone registry'si yoktur. K-01 bu modu naive kaynaklar için
  artık fail-closed yapar; gerçek borsa takvimi bağlamak kapsam dışı.
- K-01 öncesi yazılmış processed Parquet'ler hâlâ naive string tutar. Engine artık onlar için
  tahmini UTC yerine `None` döner (her çağrı noktası fail-closed işler); flaglenen revision'da
  Analyze'ı yeniden koşmak tam davranışı geri getirir.
- Polars `read_csv` timestamp kolonlarını `String` olarak infer eder (dört formatta ampirik
  doğrulandı), dolayısıyla normalize sonrası karışık-tip parquet riski yoktur.
