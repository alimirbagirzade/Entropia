...
## summary["timeframe"] market-revision metadata çözümü — ✅ LANDED (PR #137, main `22c099e`)

**BACKEND-ONLY** (migration YOK, alembic head `0021_local_auth` SABİT; **ENGINE_VERSION BUMP:
`backtest-engine-v2-position-size-limits` → `backtest-engine-v2-summary-timeframe`**; backend
**1057 → 1061**; frontend DEĞİŞMEDİ — `lib/backtest.ts` zaten `timeframe: string | null`).
Slice B'den beri sabit `None` olan `summary["timeframe"]` kapandı: `run_engine` opsiyonel
caller-resolved `timeframe: str | None = None` keyword'ü aldı (engine SAF — I/O yok; tüm çağıranlar
keyword-arg → default bit-bit geriye dönük güvenli); worker (`jobs/backtest_engine.py::run_backtest`)
pinned market revision'dan read-only `md_repo.get_base_timeframe_for_revision` (PR #55 helper,
`resolution_kind==BAR` → `resolution_value` örn. `"1m"`, immutable → reproducibility-safe) ile çözüp
geçirir. BAR olmayan (event-based/unknown) revision dürüst `None` — asla tahmin (L4). Çözüm engine
`try:` bloğunun DIŞINDA — beklenmedik DB hatası retryable job istisnası kalır, FAILED run olmaz.
ENGINE_VERSION bump execution_key namespace'ini kaydırır (eski timeframe'siz sonuç reuse edilmez —
INF-04/INF-05, PR #47/#63 emsali). +4 test (2 unit passthrough/default + 2 integration persisted
`"1m"`/`None`). Review APPROVE 0 CRITICAL/HIGH.

**Reuse anchor'ları:** `run_engine(..., timeframe=...)` (pinned metadata'dan çözülen her gelecekteki
...
6. **Deferred takipleri** — ~~`summary["timeframe"]` çözümü (market-revision metadata'sından)~~ ✅ **PR #137**;
