...
## O-16 · User Manual — bayat arama anchor'ı için kurtarma yolu (PR #444)

### Kusur

Doc 21 §7'nin "Open result / section" satırı üç şey şart koşuyor:

> Anchor must exist in current stream version; if stale, refetch then resolve. … Smooth scroll; no
> mutation. Missing anchor shows "The section is no longer available in the current manual."

§14 satır 464 aynı şeyi akış tarafından söylüyor ("Click target missing/stale ise stream refetch
then anchor retry"), acceptance **UM-18** ise sonucu tarif ediyor ("Client refetch + anchor retry;
bulunamazsa precise unavailable message"). Ayrıca §11 anti-pattern listesi (satır 1107) doğrudan
"stale anchor recovery uygula" diyor.

Kodda hiçbiri yoktu. `frontend/src/pages/UserManual.tsx:364`:

```tsx
<a key={row.chunk_id} className="manual-search-result" href={`#${row.anchor}`}>
```

Çıplak fragment linki. `grep -rn "no longer available" frontend/src` → **0 hit**. Arama indeksi
kendi snapshot'ından cevap verir (`ManualSearchPage.meta.stream_version` bunu zaten wire'da taşıyor
ve sayfa "farklı snapshot" notunu göstermek için okuyordu bile) — yani reader'ın gerisinde kalması
**beklenen** bir durumdu, sadece tıklama tarafı bunu hiç ele almıyordu. Sonuç: kullanıcı için sessiz
bir no-op; hata yok, mesaj yok, teşhis yok.

### Ne landed

`UserManual.tsx` modül düzeyinde üç yeni parça (hepsi presentation):

| Sembol | Rol |
|---|---|
| `ANCHOR_UNAVAILABLE_MESSAGE` | Doc 21 §7 metni **verbatim** sabit — yeniden yazılmaması için tek yerde pinli |
| `scrollToAnchor(element)` | `scrollIntoView?.({ behavior: "smooth", block: "start" })` — §7'nin "smooth scroll; no mutation" maddesi |
| `waitForAnchorElement(anchor)` | Sınırlı poll: `ANCHOR_RETRY_ATTEMPTS=10` × `ANCHOR_RETRY_POLL_MS=16` |

`ManualSearchNav` iki state (`recoveringAnchor`, `unavailableAnchor`) ve bir `openResult(anchor)`
kazandı; sonuç `<a>`'sı `onClick` ile `preventDefault` edilip oraya bağlandı. Parent, prop olarak
`onRefetchStream={async () => { await stream.refetch(); }}` veriyor.

Sıra doc 21 §7'nin cümlesiyle birebir: **var mı → scroll** · **yoksa refetch → tekrar dene → scroll**
· **hâlâ yoksa verbatim mesaj**.

### Üç tasarım kararı ve gerekçeleri

**1. `href` silinmedi.** Tıklama `preventDefault` ediliyor ama `href={"#"+row.anchor}` duruyor:
link kopyalanabilir, orta tıkla açılabilir, klavyeyle odaklanabilir kalıyor. `<a>`'yı `<button>`'a
çevirmek erişilebilirlik açısından bir kayıp olurdu.

**2. Poll, tek okuma değil.** Refetch'ten dönen bölüm React'in bir render pass'inden geçmeden DOM'a
girmiyor. `await refetch()` sonrası `getElementById` **tek kez** okunsaydı kurtarılabilir her anchor
"yok" görünürdü — testte de üretimde de. 10 × 16ms ≈ 160ms üst sınır; başarısız senaryoda mesajın
gecikmesi bu kadar.

**3. Refetch, sayfa-1 reset'i DEĞİL.** Sayfa `accumulate-on-load-more` çalışıyor: `frontier=null`
yapmak `sections`'ı sayfa 1 verisiyle **değiştirir** ve daha önce "Load more" ile getirilmiş bölümler
düşer. Yani reset, kurtarmaya çalıştığı anchor'ı silebilirdi. `stream.refetch()` mevcut cursor
sayfasını tazeler, biriken kuyruk yerinde kalır.

### Erişilebilirlik

`role="status" aria-live="polite"` bölgesi **koşulsuz mount**; içerik koşullu. Bölge sonradan
eklenirse ekran okuyucular değişikliği duyurmaz — bu, canlı bölge kalıbının bilinen tuzağı. Bekleme
sırasında "Refreshing the manual to locate that section…", başarısızlıkta verbatim mesaj basılıyor.

### Testler

`frontend/src/test/userManual.test.tsx` → `describe("stale anchor recovery")`, 3 case:

| Senaryo | Kurgu | Beklenen |
|---|---|---|
| Anchor canlı | `SEARCH` sonucu `sec-mdoc-2`, stream'de var | `#sec-mdoc-2` elementine smooth scroll · **stream GET sayısı sabit** · mesaj yok |
| Bayat, kurtarılabilir | `GET /manual/stream` handler'ı sayaçlı: 1. çağrı `STREAM`, sonrası `STREAM_WITH_LATE_SECTION` | tık anında `sec-mdoc-9` DOM'da **yok** → refetch → retry → `#sec-mdoc-9`'a scroll · mesaj yok |
| Hiç yok | sonuç `sec-mdoc-gone`, stream hiç getirmez | verbatim mesaj · refetch **denendi** (GET sayısı arttı) · **scroll yok** |

`stubScrollIntoView()` jsdom'un olmayan `Element.prototype.scrollIntoView`'ini, **`this` hedefini de
kaydeden** bir double ile kuruyor. Böylece assertion "scroll çağrıldı" değil, "**doğru bölüme**
scroll çağrıldı" oluyor — bir sonraki değişiklik yanlış elemente kayarsa test kırılır. `afterEach`
prototipten siliyor.

Doğrulama: **vitest 622/622 passed** (60 dosya; `--no-file-parallelism` ZORUNLU) · `npm run
typecheck` (`tsc -b --noEmit`) temiz · `eslint src --max-warnings 0` temiz.

### Dokunulmayanlar (V18 UI kuralı)

Route path, react-query key (`["manual", …]`), OCC token (`expected_stream_version` /
`expected_head_revision_id`), `Idempotency-Key`, SSE taksonomisi, `lib/manual.ts` veri mantığı,
`app/nav.ts` — **hiçbiri değişmedi**. Backend'e tek satır girmedi; migration yok, alembic head
`0039_backtest_run_cancellation` aynı, `ENGINE_VERSION` aynı. Değişen tek imza modül-içi
`ManualSearchNav`'ın prop'u.

### Dürüst sınırlar

- **Yüklenmemiş sayfa.** Retry yalnız stream query'sinin döndürdüğünü bulabilir. Anchor hâlâ
  getirilmemiş bir "Load more" sayfasındaysa mesaj yanlış tarafa düşer (bölüm aslında var). Spec'in
  reçete ettiği kurtarma tam olarak "refetch then resolve"; daha fazlası (anchor'ı sunucuya sorup
  sayfayı hedefleyerek getirmek) yeni bir okuma yüzeyi gerektirir ve yetkilendirilmiş kapsam
  dışıydı. Kod içinde yorum olarak duruyor.
- **Kenar çubuğu bölüm listesi** (`UserManual.tsx:118`) çıplak `<a href>` olarak bırakıldı. Bu
  linkler render edildikleri **aynı** snapshot'tan türüyor — bayatlayamazlar. Doc 21 §7 satırının
  başlığı "Open result / section" olsa da riski taşıyan taraf arama indeksidir; olmayan bir kusur
  için kod eklenmedi.
- **Refetch hatası verdict değildir**: `catch` yutuyor, kararı yine render edilmiş olana bakan retry
  veriyor. Bir ağ hatası "bölüm silindi" diye raporlanmıyor.
- Yeni **E2E** senaryosu yazılmadı; kapsama vitest bileşen düzeyinde. Kurtarma tamamen istemci
  tarafı olduğu için gerçek tarayıcı ek bir davranış kanıtlamıyor.
