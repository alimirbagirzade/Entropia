<!-- doc-status: current -->
> Bu belge **ADIM 34 kapanışında** yazıldı. En altındaki *paste-ready resume prompt*
> temiz bir oturuma yapıştırılacak devam tohumudur.
> Sayısal otorite: `docs/generated/repository_facts.md` (üretilmiş, CI'da `--check`).

# ADIM 34 LANDED — RC §6.7 / P6-ek + P6-6: harness fail-fast
...
**CI kapısı olan** bir regresyon testine bağlandı. Rapor kaydı: **§6.7.3** + §6.2 notu.
...
BASE: origin/main (DOĞRULA — ADIM 34 / P6-ek + P6-6 merged olmalı; `git fetch` + `gh pr list`)

OTURUM BAŞLANGICI
  1. git fetch; git log --oneline origin/main -6; gh pr list --state all
     → ADIM 34 (bounded.sh + fail-fast harness) gerçekten indi mi? Handoff BAYATTIR, doğrula.
  2. Oku: docs/ADIM34_LANDED_KICKOFF.md (bu belge) → docs/STAGE2_HANDOFF.md §Next
