...
## DOKÜMAN ONARIMI — PR #590'ın sildiği ADIM 18 kaydının geri yüklenmesi (2026-08-06)

**Ne oldu.** `cdb3ab1` (PR #586) bu dosyaya `## ADIM 18 — run_portfolio faz döngüsü; ADR 0002
Accepted` kaydını (208 satır) eklemişti. **`c3f5673` (PR #590) onu sildi** — o commit'in bu
dosyadaki diff'i **211 silme / 0 ekleme**:

```
git show c3f5673 --stat -- docs/PROJECT_HISTORY.md
 docs/PROJECT_HISTORY.md | 211 ------------------------------------------------
```

PR #590, `d7fe432` (PR #584) ile **aynı başlığı ve aynı gövdeyi** taşıyan mükerrer bir docs
PR'ıydı; branch'i ADIM 18 kaydından önceki bir duruma dayanıyordu, bu yüzden merge kaydı sessizce
geri aldı. Ne PR #590'ın gövdesi ne de commit mesajı bir silme bildiriyor.

**Neden hiçbir kapı yakalamadı.** Hiçbir CI job'ı `docs/` markdown'ını okumaz. Bu dalganın en
önemli mimari slice'ı — `run_portfolio` faz döngüsü ve ADR 0002'nin `Accepted`'a geçişi — bu
yüzden tarihçede haftalarca **hiç görünmedi**, üstelik sonraki dört docs PR'ı (#595, #596, #603,
#607, #611) bu dosyaya yazarken de fark edilmedi.

**Onarım.** Aşağıdaki bölüm `ba586c5:docs/PROJECT_HISTORY.md`'den **birebir** geri yüklendi; tek
kelimesi değiştirilmedi. `docs/STAGE2_HANDOFF.md`'deki landed girdisi hiç etkilenmemişti, kurtarma
kaynağı oydu. Tekrarlanabilir komut:
...
tarihli anlık görüntülerdir ve geriye dönük yeniden yazılmaz.

**Süreç dersi:** bir docs PR'ı sessizce başka bir docs PR'ını geri alabilir ve hiçbir otomatik
kapı bunu görmez. **Docs PR'ı açarken kendi diff'inin silme satırlarına bak:**
