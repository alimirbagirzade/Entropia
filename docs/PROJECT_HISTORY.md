...
**Neden.** `docs/implementation/v18_visual_traceability.md` §2 Bucket 3'te F-07 satırı
`Not started` + "overlaps P-11/12/16" notuyla duruyordu. P-11/P-12/P-16 landed olduğu için satır
**hiçbir yönde güvenilir değildi** — "kalıntı yok" da "kalıntı var" da varsayımdı. Bu slice
varsayımı ölçümle değiştirir.

**Yöntem.** Test-dışı her `frontend/src/**/*.tsx` üzerinde JSX **text-node** taraması: görünür
metin konumundaki (`>{expr}` / `{expr}<`; attribute ve fonksiyon imzaları hariç) her `{expr}`
içinde `*_id` alanı okuyanlar. **31 dosyada 161 render.** Ham sayı bulgu DEĞİL — F-07'nin kendi
acceptance'ı ID'leri destek/denetim yüzeylerinde **açıkça serbest bırakıyor** ("Keep copyable IDs
in advanced detail for support/audit"; acceptance = "No common task requires recognizing an opaque
identifier"). Bu yüzden her render **"yaygın bir görev için birincil kimlik"** (ihlal) ya da
**"gelişmiş/denetim detayı"** (serbest) olarak sınıflandırıldı.

**Sonuç: kalıntı VAR — ama iddia edilen yerde değil.** P-11/P-12/P-16 gerçekten landed:
`RationaleFamilies.tsx:637` paketi `package_name ?? package_root_id` ile adlandırıyor (`20ccacc`),
Results History satırı P-12 digest'ini (completed-at / timeframe / symbol) taşıyor (`3c6887c`),
Portfolio sleeve satırları/picker/örnek satırı `ItemLabel` ile ad gösteriyor (PR #375).
Traceability'nin 10 / 13 / 16 numaralı satırlarındaki **"raw ids" gerekçeleri bayattı** ve
düzeltildi (satırlar PO D-4/D-5 ve A-06 nedeniyle OPEN kalır — imza ayrı bir kapı).

**Düzeltilen (presentation-only, iki kalıntı — ikisi de P-11'in süpürdüğü sayfada):**

1. `Portfolio.tsx` `IssuesTable` — bir validation issue'nun **hangi kompozisyon öğesine** ait
   olduğunu yalnızca çıplak `composition_item_id` ile gösteriyordu. Sayfa `labelByItem` haritasını
   diğer tüm yüzeyler için zaten hesaplıyordu; bu tabloya hiç geçirilmemişti. `IssuesTable` artık
   `labelByItem` alıyor ve `ItemLabel` (ad birincil, id ikincil bağlama anahtarı) render ediyor.
   Her iki çağrı yerine de geçirildi — `SaveResultCard`'a `draftQuery.data.draft.entries` üzerinden.
2. `Portfolio.tsx` örnek satırı — `?? sleeve.composition_item_id` fallback'i **ham `mbi_` id'yi
   birincil ada terfi ettiriyordu**, kendi sözleşme yorumuyla çelişerek (`allocation.ts:57-60`:
   "NEVER the raw mbi_ id"). Yeni `UNLABELLED_ITEM` sabitine düşüyor; id ikincil anahtar kalıyor.

Yeni modül-içi yardımcı `labelsByCompositionItem()` ad-hoc map kurulumunu birleştirdi.
**Hiçbir** route path / react-query key / OCC token / Idempotency-Key / hook / SSE taksonomisi /
`lib/*.ts` veri mantığı / `app/nav.ts` girdisi değişmedi.

**Kabul testi + RED kanıtı.** `portfolio.test.tsx` → *"names the item a validation issue points at,
instead of a bare id (F-07)"*; etiketin **issue satırının içinde** olduğunu `within` ile doğrular,
id'nin de orada kaldığını ayrıca doğrular. Yalnızca `IssuesTable` render'ı süpürme-öncesi
`<code>{issue.composition_item_id}</code>` haline geri alındığında **bu test ve yalnızca bu test**
kırmızıya döndü; sonra geri yüklendi.

**Kapatılmayan dört kalıntı (dürüst sınır — backend display DTO gerekir).** Bunlar gerçek F-07
ihlali ama DTO'ları **insan etiketi taşımıyor**; tarayıcıda ad uydurmak tam olarak bulgunun
yasakladığı şey ("never reconstruct names from IDs in the browser"). F-07'nin kendi reçetesi
*"Add display DTOs at query boundaries"* — bu bir **backend query değişikliğidir**, presentation-only
slice'ın dışında:

- `PreCheck.tsx:124` — istek seçici tablosunun "Request" sütunu çıplak `request_id`; Type/Source/State
  tür bilgisi, ad değil. Kendi isteğini seçmek = opak tanımlayıcı tanımak. (F-07'nin adıyla saydığı sayfa.)
- `Library.tsx:1259`, `1274` — import job satırları yalnızca `import_job_id`.
- `ResultDetail.tsx:420`, `589` — per-item breakdown `{item_kind} <code>{item_id}</code>`, leave-one-out
  `Without <code>{entry.item_id}</code>`. Result **immutable ve pinli**; canlı kompozisyonun etiketlerini
  join etmek, öğeleri o zamandan beri değişmiş bir sonucu **yanlış etiketler** — etiket manifest'ten gelmeli.
- `ReadyCheck.tsx:291` — issue satırlarında çıplak `scope_id` (bu slice'ın Portfolio'da kapattığı kusurun
  aynısı). Rapor immutable + `is_current` izlenen; **bayat** bir raporu canlı kompozisyondan etiketlemek
  aktif olarak yanlış olurdu.

`ResultsHistory.tsx:170` bilerek ihlal sayılmadı: bir backtest result'ın kullanıcı-verili adı yoktur ve
P-12 sonrası satır digest taşır — id tek ayırt edici değil. Digest biçimi PO'ya ait (D-5).

**Doğrulama.** `eslint` ✓ · `tsc -b --noEmit` ✓ · `vitest` **608/608** (önce 607; +1 yeni case).
**Ortam tuzağı doğrulandı:** varsayılan paralel koşuda 55 test "timed out in 5000ms" ile düştü;
aynı dosyalar tek tek ve tüm suite `--no-file-parallelism` ile **608/608 yeşil** — düşüşler worker
contention'ı, kod değil. Backend'e dokunulmadı → **ENGINE_VERSION bump YOK** (motor davranışı
değişmedi), **migration YOK**.

**F-07 hâlâ Complete DEĞİL.** Bu slice sunum katmanını kapatır; §4.4'teki dört yüzey bir okuyucudan
hâlâ opak tanımlayıcı tanımasını istediği sürece bulgu bütün olarak kapanamaz. Ayrıntılı kanıt:
