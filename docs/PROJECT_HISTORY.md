...
## ADIM 16 — `run_engine`'in bar döngüsü resumable stepper'a çıkarıldı (PR #602)

**Ne indi.** `run_engine`'in ~2400 satırlık gövdesinin içine gömülü **1355 satırlık bar
döngüsü** askıya alınabilir bir stepper'a çıkarıldı. Fonksiyonun bar döngüsüne kadarki yarısı
`_build_stepper(...)` oldu (`engine.py:779`) ve bir `_ItemStepper` (`engine.py:756`, frozen
dataclass) döndürüyor: `step(bar)` / `finalize()` / `output()` / `open_position()` + canlı
`ledger` ve `ctx`. `run_engine` **imzasını, docstring'ini ve semantiğini** koruyup onun üzerinde
**dokuz satırlık bir sürücü** oldu (`engine.py:3245`).

**Neden.** ADR-0002 §12 ADIM 16'yı resmen **SKIPPED** işaretlemişti — faz döngüsü (`run_portfolio`,
ADIM 18) aynı yere öbür taraftan varmıştı. Ama aynı §12 düzeltme notu stepper'ın **worker call
site'ının ön koşulu** olarak kaldığını söylüyor: gerçek engine ile desteklenen bir
`ItemParticipant` tek bir item'ı verilen `t`'ye ilerletebilmek zorunda, ve döngü o dev
fonksiyonun içinde nested iken `engine.py`'de bunu yapabilecek hiçbir şey yoktu. Yani ADIM 16
"atlanmış bir adım" değil, **ertelenmiş bir borçtu**; bu PR onu ödedi.

**Bu PR, engine-destekli `ItemParticipant` çiftinin PR A'sıdır.** Worker'a dokunulmadı.

**Kabul kriteri: 46 golden digest ve BAŞKA HİÇBİR ŞEY** (ADR §15 **R-4** — *"an ADIM 16 refactor
that silently changes a single-item number — 46-digest invariance is the gate; no other
assertion is trusted"*). **46/46 kımıldamadı.**

**Saf refactor iddiasının dayanağı — ikisi de yeniden üretilebilir:**

- **Taşınan her satır verbatim.** Edit sonrası taşınan aralıklar `HEAD` ile byte-byte
  karşılaştırıldı: setup **955** satır, step gövdesi **1351**, end-of-data settlement **44**,
  output assembly **15** — hepsi birebir aynı. Formatter dedent sonrası tek satıra sığan **tam
  bir** `max(...)` çağrısını topladı; başka fark yok.
- **`nonlocal` bloğu ölçüldü, tahmin edilmedi.** Döngü gövdesi üzerinde bir AST geçişi bar'lar
  arasında taşınan **tam on** ad bildirdi: `current_day`, `exit_touch`, `funding_idx`, `pending`,
  `position`, `prev_entry_signal`, `prev_scale_signal`, `scale_signal`, `working_limit`,
  `working_stop`. Gövdenin bağladığı **diğer 83** ad, definite-assignment analiziyle her yolda
  okunmadan önce yazıldığı kanıtlanarak step'e local bırakıldı. Analiz yanlış olsaydı hata modu
  **gürültülü bir `UnboundLocalError`**'dur — sessizce farklı bir sayı değil.

**Digest'lerin göremediği yarı — yeni test.** `run_engine` stepper'ı **tek generator'dan
kesintisiz** besliyor, yani sessizce per-bar local'a dönmüş bir taşıyıcı ad **yine de** kayıtlı
digest'i üretebilirdi. `tests/unit/test_backtest_engine_stepper.py` (**4 test**, 216 satır) aynı
senaryoları **çağrı başına bir bar**, her bar çiftinin arasında askıya alarak replay ediyor ve
digest özdeşliğini iddia ediyor — taşıyıcı ad başına bir vaka ile: duran limit emri, hiç
dokunmayan limit, tetiklenmemiş stop, ladder'layan pozisyon, iki kayıt boyunca funding ödeyen
tutulan pozisyon, blackout gününü aşan sinyal. Ayrıca batch sınırının gözlemlenemezliği,
askı boyunca `open_position()` doğruluğu ve **ilk bar'dan önceki** fail-closed
`UnresolvedStrategyError`.

**Değişen dosya iki:** `domain/backtest/engine.py` + yeni test. **Migration yok, model yok,
OpenAPI yok.** `ENGINE_VERSION` = `backtest-engine-v18-gap-adjusted-stop-fill` (`manifest.py:126`)
**bilerek bump EDİLMEDİ**; containment guard kapalı (`SHARED_ALLOCATION_STATUS = future_dev`);
manifest policy alanı eklenmedi — üçü de ADIM 20.

**CI (#602): 8 job pass**, 2 heavy job skipped; `Backend — lint, type, test` 44m09s. PR gövdesinde
bildirilen suite: **3699 passed · 4 xfailed · 0 failed**, coverage **%93.29** (kapı ≥90;
`engine.py` %95.1). 4 xfail bilinen katı küme (#556 ×2, #557, #558) — değişmedi.

**DÜRÜST SINIR.** `_ItemStepper` / `_build_stepper` **module-private ve `__all__`'da DEĞİL**:
`engine.py` dışından onları çağıran yok. Bu PR'ın tek gözlemlenebilir etkisi `run_engine`'in
döngü yerine sürücü olmasıdır. Tüketiciyi veren şey **PR B**'dir: `_ItemStepper` üstüne
`portfolio_engine.ItemParticipant` adaptörü + `jobs/backtest_engine.py:298` call site'ı
(`run_portfolio`, **yalnız >1 item**; tek item `run_engine`'de kalır — ADR §3.2). O PR'da 9
`portfolio.*` digest'inin kımıldaması **beklenir**, 37 non-portfolio digest kımıldamamalıdır.
İkisi tek PR'da **birleştirilmedi**: ADR §15 R-4'ün restructure/re-price ayrımı ADIM 16
atlandığında bir kez kaybedildi; ikinci kez kaybedilirse kımıldayan bir digest'i atfedecek
hiçbir şey kalmaz.

**Bu kaydın sınırı.** Yerelde tam suite ile bağımsız doğrulama **yapılmadı**; yukarıdaki 3699 /
%93.29 sayıları #602'nin kendi PR gövdesinden alınmıştır, bağımsız otorite **CI koşusudur**.
