...
- **GateGuard:** write NEW files via Bash heredoc (`cat > f << 'PYEOF'`) -> gate-free; an
  EDIT to an existing file triggers fact-force (present 4 facts: importers / affected
...
- **Landed:** Stages 0-4b + **5a — RUN + Backtest Results core (doc 15)**. `main` after
  PR #16 = `974c379`; alembic head = `0014_backtest_run_result`.
- **Next:** **Stage 5b — Results History (doc 16) + Arrange Metrics (doc 17) + doc-15
  deferred** (Export + artifact cursor query). Branch `feat/stage-5b-results-history`;
  migration `0015_*`. Full handoff: `docs/STAGE5B_KICKOFF.md`.
