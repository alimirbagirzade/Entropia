...
| **OD-2** | **(a)** — carry the last closed bar's close forward with a declared `stale_after` bound and a diagnostic counter. | **Built (ADIM 132), bound (ADIM 135).** `run_portfolio` marks the book once per tick at `PV`, inside the frozen window, via `PortfolioLedger.valuation`; the bound is `MARK_STALE_AFTER_MS` (900 s), recorded in the manifest as `carry_forward_bounded_v1`, and the diagnostic counter is `PortfolioValuation.stale_refused_items`, published per run as `diagnostics["mark_staleness"]`. **`E(t)` is still realized-only, exactly as `PortfolioLedger` defines it** — the mark is REPORTED beside equity and never folded in, so no unmarked position can silently enter equity; pinned by `test_oracle_od2_mark_binding.py::test_the_mark_never_moves_e_of_t`. Scope is **product-owner decision (b), 2026-08-28** (`docs/decisions/closure_od2_mark_production_binding_2026-08-28.md`): diagnostics only — the figures reach neither provenance nor `execution_content`, so reproduction identity is untouched and no `ENGINE_VERSION` bump was required. *Was: "Not built. `run_portfolio` marks nothing … Building the mark policy is a prerequisite of ADIM 20, not of the loop." — accurate until ADIM 132; ADIM 20 (`C9`) has since landed.* |
