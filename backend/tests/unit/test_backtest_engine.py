...
def test_engine_execution_key_namespace_shifts_with_the_engine_version() -> None:
    ...
    # though no price or metric did. F-07 §4.4 then bumped it to -per-item-labels: the
    # composite per-item / leave-one-out rows gained ``item_label``.
    built = _manifest("btrun_A", "snap_A", "2024-01-01T00:00:00Z")
    expected = "backtest-engine-v18-min-n-filtered-events-per-item-labels"
