...
// NOTE (I-16a, attempted then reverted 2026-07-29): the row deliberately renders
// the raw `result_id` as a `<code>` binding key and NOT the projection's
// `display_title`. That field ships as the literal string
// `f"Backtest Result {result_id}"` (`application/queries/results_history.py::_row_dto`),
// so promoting it into the name slot printed the id twice and named nothing. What
// actually discriminates a result row is the completed-at / timeframe / symbol trio
// rendered beside the id since P-12. Full rationale and the rejected alternatives:
