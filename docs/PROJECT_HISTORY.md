...
## T-01 · Testsiz kritik modüller — audit-log read model, role matrix, settlement currency (PR #422)

**Branch** `feat/t1-untested-critical-modules` · **commit** `8433ff7` · **merge** `b54d7d7`
(2026-07-28T10:07:35Z) · **migration YOK** (alembic head o an `0039_backtest_run_cancellation`) ·
**`ENGINE_VERSION` değişmedi** (`backtest-engine-v18-funding-step-order`) · **kaynak koda
dokunulmadı** — diff tamamen `tests/` altında (3 dosya, +748 satır, 55 test).

### Ampirik kapsam doğrulaması — backlog maddesi kısmen yanlıştı

Slice'ın ilk işi iddiayı sınamaktı; 3 iddiadan 1'i yanlış, 1'i yarı-yanlış çıktı.

| Modül | İddia: "hiçbir test dosyasında sembol referansı yok" | Ampirik sonuç |
|---|---|---|
| `application/queries/audit_log.py::list_audit_events` | **doğru** | Hiçbir test dosyasında geçmiyor. Tek kapsama route'un 403 reddi (`tests/contract/test_identity_and_gating.py:80`) — o da `require_admin` ile **sorgudan önce** döner, yani projeksiyon gövdesi CI'da hiç çalışmamıştı. |
| `domain/admin_panel/role_matrix.py::build_role_matrix` | **YANLIŞ** | İki test zaten referans veriyordu: `tests/unit/test_admin_panel_taxonomy.py::test_role_matrix_projection` (satır kümesi + revision + 4 nokta hücre) ve `tests/integration/test_panel_management_logs.py::test_role_matrix_admin_only` (yetki kapısı). Ancak **20 rol×yetenek hücresinden 14'ü** pinlenmemişti. Sessiz-regresyon riski gerçekti; "testsiz" nitelemesi değil. |
| `application/queries/allocation_currency.py::resolve_settlement_currencies` | **doğrudan referans yok** | `test_allocation_fx_dependency.py` üzerinden dolaylı koşuyordu ama **yalnız STRATEGY doğrudan-payload dalı**. doc 02 §7.1 mirror deref, TRADING_SIGNAL ve TRADE_LOG dalları hiç çalışmamıştı. |

**Brief'in bir isteği bu modüle ait değildi.** Görev audit-log için "filtre kombinasyonları"
istiyordu; `list_audit_events` filtre almaz — imzası yalnız `PageParams(cursor, limit)`. Filtreli
log yüzeyi **ayrı bir modüldür**: Admin Panel `list_log_events` (doc 19; `domain/admin_panel/log_taxonomy.py`,
log cursor `encode_log_cursor`/`decode_log_cursor`) ve o zaten test edilmiş. Olmayan bir filtreye
test yazmak "yeni davranış EKLEME" yasağını çiğnerdi; modülün gerçekte yaptığı iş kilitlendi.

### Ne kilitlendi

**`tests/unit/test_role_matrix_contract.py` (30 test).** Kanonik grid, **modülden türetilmeyen**
literal `_CANONICAL_ROWS` tablosuna karşı hücre hücre parametrize (`_ALL_CELLS`, 20 hücre) — politika
iki yerde birden değişmedikçe sapma kırmızı yanar. Ayrıca: satır sırası (admin, supervisor, user,
agent — sunum sözleşmesi), her satırın her kolonu bildirmesi, `columns` listesinin yetenek
anahtarlarıyla birebir olması, `ROLE_MATRIX_REVISION == "2026-06-role-matrix-v1"` pini, agent'ın tek
sistem aktörü ve tek atanamaz rol olması, trash/role_assignment'ı yalnız Admin'in yönetmesi, ve
`build_role_matrix()`'in **çağrı başına kopya** döndürmesi (bir çağıran `_ROWS`'u mutasyona uğratamaz).

**`tests/integration/test_audit_log_read_model.py` (11 test).** Yeniden-eskiye sıralama + sayfa
tavanı; `next_cursor`'ın **yalnız daha fazlası varken** dolması ve sayfayı tüketince `None` olması;
cursor'ın **dışlayıcı** olması (kendi olayını sayfaya koymaz); 7 olayı 2'şerli sayfalarla gezip her
olayı **tam bir kez** gören sınırlı cursor yürüyüşü; projekte edilen 11 alanın **tam sözlük eşitliği**
(`reason` gibi tabloda olup projeksiyonda olmayan alanlar dahil — sızarsa kırmızı); null kolonların
atlanmak yerine `null` olarak yayımlanması; **öznesi artık var olmayan olayın** listelenmeye devam
etmesi (audit standalone + immutable, delete/purge'ü aşar); route'un Admin kapısı **iki yönde**
(admin → 200 + `{data, meta:{cursor, has_more}}` zarfı, user → 403 `ACCESS_DENIED`).
Route testleri `request_context`'i **gerçek Postgres session'ı** ile override eder — contract
testlerinin `_DummySession` deseninden farkı budur ve route+query'yi birlikte koşturur.

**`tests/integration/test_allocation_settlement_currency.py` (14 test).** Dört çözümleme yolunun
tamamı: strategy doğrudan `data.instrument_id`; strategy **§7.1 mirror deref**
(`payload.strategy_revision_id` → `StrategyRevision.payload.data.instrument_id`); TRADING_SIGNAL
(`resolve_signal_revision`); TRADE_LOG (`resolve_trade_log_batch`). Ve `None` veren **her** belgelenmiş
yol: payload'da `data` yok / `instrument_id` boş string, mirror hedefinde instrument yok, pinlenen
revision hiç yok, normalized revision veya record batch pin yok, instrument satırı yok,
`settlement_asset` null. Artı eşleme şekli: her item için giriş (çözülmeyen dahil), boş kompozisyon → `{}`.
Bu dalların **hepsinin sessiz** olması testin gerekçesidir: çözülmeyen bağ `None` verir, saf FX
kontrolü o item'ı **SKIP** eder — bozuk bir dal blocker'ı sessizce pass'e çevirir.

**Test kurma tekniği (yeniden kullanılabilir).** `MainboardWorkingItem` session'a **eklenmeden
detached** kurulur — resolver yalnız `item_id`/`item_kind`/`pinned_revision_id` okuduğu için
workspace/registry/composition FK zincirini kurmaya gerek kalmaz; pinlenen revision'lar ise gerçek
satırlardır. Mirror şekli `strat_repo.create_strategy` + `append_strategy_revision` +
`mb_repo.append_work_object_revision` ile, strateji doğrulama/reference makinesine hiç girmeden üretilir.

### Mutasyon kanıtı — testler gerçekten ısırıyor

Üç kaynak modüle kasıtlı birer kusur enjekte edildi; her biri **tam olarak bir** testi düşürdü,
sonra dosyalar HEAD'e geri alındı (`git status` yalnız 3 yeni test dosyası gösterdi):

| Enjekte edilen kusur | Düşen test |
|---|---|
| `allocation_currency.py`: `if mirror_ref:` → `if False:` | `test_resolves_a_strategy_pin_through_the_mirror_strategy_revision` |
| `audit_log.py`: `if has_more and page` → `if page` | `test_returns_no_cursor_when_the_page_exhausts_the_log` |
| `role_matrix.py`: supervisor `view_use` `shared_and_published` → `all` | `test_every_role_capability_cell_matches_the_canonical_policy[supervisor-view_use-…]` |

### Verify

`ruff check` temiz · `ruff format --check` 624 dosya temiz · `mypy src` 356 dosya sorunsuz ·
izole DB'de (`TEST_DATABASE_URL=…/entropia_test_t1`) tam backend suite **2285 passed, 0 failed,
0 errors, 0 skipped** · CI **6/6 yeşil** (backend 34m14s, frontend, dev-auth E2E, Docker-Compose
E2E, A11Y axe-core, Docker build). L1 FK insert-order proof / alembic up-down-up / migration↔model
parite: **N/A** — yeni `create_*` yok, migration yok, şema değişmedi.

### İki ortam dersi (ikisi de bu slice'ta gerçekten ısırdı)

1. **`pytest … | tail` KULLANMA.** Boru hattının exit code'u `tail`'in olur, pytest'in değil.
   Bu slice'ta 20 ERROR veren bir tam-suite koşusu `exit code 0` gördüğü için bir tur boyunca
   "yeşil" sanıldı ve öyle raporlandı. Çıktıyı dosyaya yaz, `$?`'i ayrı oku.
2. **Tam suite koşarken `uv sync`/`uv run` çalıştırma.** O 20 ERROR'ın nedeni buydu: suite koşarken
   paralel `uv` komutları venv'i ortadan yeniden kurdu. Sessiz yeniden koşuda sıfır hata; ilgili
   dosya (`test_condition_plan_resolution.py`) izole koşuda 23/23 geçti.

Ek olarak `main` bu slice merge edilirken **altımızdan kaydı**: paralel oturumlar K-09
(#423/#425/#426/#424), K-10 (#428/#429) ve #427'yi indirdi. T-01'in kendi içeriği etkilenmedi
(test-only, çakışmasız). Kapanış sırasında bir yanlış tespit düzeltildi: oturumun worktree tabanı
`eff8ffe` olduğu için oradaki CLAUDE.md `0035` diyordu ve bu bir an "4 migration bayat" sanıldı —
oysa main'deki CLAUDE.md O-06 kapanışında zaten `0039`'a güncellenmişti. Doküman bayat değildi,
oturum tabanı eskiydi; STALE-BY-DEFAULT kuralı worktree kopyasına da uygulanmalı.

### Açık iş / dürüst sınır

- Filtreli `list_log_events` (doc 19) yüzeyinin kapsam **derinliği denetlenmedi**; bu slice ona dokunmadı.
- K-09/K-10 extraction'ından doğan yeni engine modüllerinin (order matching, stop resolution,
  position sizing, leverage, cost model, funding decision, `_Ledger`) test kapsamı **denetlenmedi** —
  T-02 adayı. #427'nin golden-digest guard'ı uçtan uca davranışı pinler, modül-içi dalları pinlemez.
- Bu slice **yalnız üç modülü** kilitler; repodaki diğer test boşlukları hakkında hiçbir iddiada bulunmaz.
