...
## Doc truth-repair — dört authority dokümanı yeniden ölçüldü (PR #415)

**Ne landed.** DOC-01/02/03 + I-06 denetimi: authority-order dokümanları, taze bir oturumu
**bitmiş işi yeniden yaptıracak** kadar bayatlamıştı. Dört doküman `origin/main` @ `eff8ffe`'ye
karşı yeniden ölçüldü. Merge: **`36957ad`**, CI **6/6 yeşil**. **Kod/migration/`ENGINE_VERSION`
dokunulmadı** — `git diff --name-only` çıktısında `.md` dışı dosya yok.

**En yüksek riskli düzeltme — `docs/STAGE_R3_KICKOFF.md`.** Authority order'da **1. sırada**
okunuyor ve §"Remaining R3 backlog" başlığı altında **merged** beş kalemi sıradaki iş gibi
listeliyordu: D-4 (#375) · P-05 (#376) · P-06 (#377) · P-14 (#378) · P-09 (#379). Ayrıca W2
tablosunda #371/#372/#373 hâlâ "open (green)" yazıyordu — üçü de merged. **SUPERSEDED banner**
eklendi; §"Working-loop method" **korundu** (hâlâ geçerli); §"Paste-ready resume prompt"
**yapıştırma** olarak işaretlendi (bayat `0035_portfolio_rules` + `-capability-matrix` beklentisi).

**`CLAUDE.md` §Next'in taşıyıcı iddiası yanlıştı.** "Kalan tek blokaj product-owner imzası …
imza olmadan hiçbir satır Complete olmaz" diyordu. **İmza 2026-07-22'de atılmış**
(`docs/implementation/v18_final_acceptance.md:155-169`, D-1…D-9) ve imzanın beklettiği FIX(R3)
kalemlerinin **hepsi landed** (#368–#373, #375–#379). §Next gerçek kalan dört işe göre yeniden
yazıldı. Ayrıca alembic head `0035_portfolio_rules` (35) → `0038_backtest_run_event` (38) ve son
dalga K-serisinden O-serisine taşındı.

**`docs/CODEMAPS/DATA_MODEL.md` — iki sayı hatası.** (1) Tablo sayısı **63 → 102** (%63 eksik
beyan). `rationale_family_revision` bir `_root / _revision` kısaltmasının arkasında
**greplenemez** haldeydi → kendi satırına açıldı; `comm` ile 102 tablonun tamamının adlandırıldığı
doğrulandı. (2) **Brief'te olmayan, taşıyıcı ikinci hata:** doküman "tüm repoda yalnızca **8**
açık `ForeignKey(...)` bildirimi var" diyor, hemen altında **9 satır** listeliyordu. Gerçek:
**134 bildirim / 25 model dosyası**. Bu iddia CLAUDE.md'nin *"her yeni `create_*` için L1 FK
insert-order proof"* kuralının **gerekçesiydi** — **kural değiştirilmedi**, gerekçe yeniden
yazıldı: çok sayıda cross-aggregate `*_id` hâlâ DB constraint'siz mantıksal bağ olduğu için
insert sırası şemadan bütünüyle türetilemiyor. Her iki sayı için **yeniden üretme komutu dosyanın
içine gömüldü**.

**`docs/POST_V1_SPEC_GAP_BACKLOG_ROUND3.md` — 9 maddenin 8'i landed.** Gövdedeki "grep = 0"
kanıtlarının çoğu artık üretilemiyor: S9 · S1 (0 → **11 hit**) · S8 (0 → **172**) · S4 (0 → **72**)
· S6 (0 → **14**) · S2 (`source_scan.py` var) · S3 (migration `0031` + 5 katman) · S7 (`:276`
artık gerçek ifade). **Gerçekten açık ayrıldı:** S5 (**dört alt-slice de** — sekiz config
token'ının hiçbiri yok, yalnız `logic_based_scaling`) + S-L1…S-L6. **S-L7 fix değil karar olarak
kapandı**: legacy nav etiketleri silinmiş, yokluğu `nav.test.tsx:95-96` regresyon testiyle
pinlenmiş.

**Denetimin kendi promptu da bayattı — üç iddia tutmadı.** (1) CLAUDE.md'de `658db36` /
"F-05 (PR bekliyor)" **hiç geçmiyordu**; o bölüm zaten güncellenmişti, asıl bayat olan alembic
head + son dalga + §Next idi. (2) Tablo sayısı 100 değil **102**; gövde 99'u değil **101**'ini
adlandırıyordu. (3) S-L1 satır kayması `964→1034` değil **`964→1277`**. **Ders:** "şu dosyada şu
yazıyor, düzelt" biçiminde gelen görevde önce **metnin gerçekten orada olduğunu** doğrula.

**Dürüst sınır.** Brief kaynak olarak `docs/audit/audit_report.md`'yi referans vermeyi istiyordu;
**bu dosya çalışma ağacında da git geçmişinin tamamında da yok**
(`git log --all --diff-filter=A -- 'docs/audit/*'` boş) — §O-03 kaydı zaten yokluğunu belirtmişti.
Her düzeltme bunun yerine yeniden üretilebilir bir **komuta, PR numarasına veya `file:line`'a**
dayandırıldı. Denetimin "25+ kod" iddiasının kalanı hakkında bu kayıt bir şey söylemiyor.

**Kapanışta fark edilen (bu slice'ın en önemli çıktısı).** #415 merge olduktan **sonra main 14 PR
daha aldı** (#416–#429: T-2, O-14, I-17, O-06, K-09 a–e, T-1, K-10a). Yani #415'in yazdığı
§Current position **bir gün içinde yeniden bayatladı**. Bu, CLAUDE.md'nin STALE-BY-DEFAULT
kuralının teorik değil **ölçülmüş** bir gerçek olduğunu gösteriyor: bu repo hızında tek seferlik
bir doğrulama yetmiyor.
