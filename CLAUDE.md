...
> **%93.29**; CI 8/8. `ENGINE_VERSION` ve containment'a dokunulmadı.
> **Next:** **PR B** — `ItemParticipant` adaptörü + `jobs/backtest_engine.py:298` call site
> (`docs/ADIM16_STEPPER_LANDED_KICKOFF.md`). **AÇIK kusur:** `apps/worker/actors.py` event-loop
> hatası durable job'ı kalıcı mahsur bırakıyor (ölçüldü, düzeltilmedi).
