...
**Denetim iddiası.** "Tick/Trades ve Spread için coverage hiç üretilmiyor ama registry dolu
gösteriyor." İddianın **birinci yarısı doğru, ikinci yarısı yanlış** — ampirik doğrulama:

- **Doğru:** `domain/market_data/validation_rules.py::evaluate_cross_row` içindeki coverage dalı
  üç koşulu birden arıyordu — `market_data_type == MarketDataType.OHLCV` **VE**
  `resolution_kind == ResolutionKind.BAR` **VE** `cadence_seconds(resolution_value) is not None`.
  Üçü birden sağlanmadıkça `coverage = ()` dönüyor, `jobs/market_data.py`'nin
  `for segment in cross.coverage: md_repo.add_coverage_slice(...)` döngüsü hiç dönmüyordu.
  Tick/Trades ve Spread/Execution için `dataset_coverage_slice` satırı **hiç oluşmuyordu**.
- **Yanlış:** registry "dolu göstermiyordu". `summarize_coverage_for_revisions` slice'ı olmayan
  revision'ı map'ten **çıkarıyor** (P-09), `queries/market_data.py::_revision_dict` `coverage: null`
  yazıyor, `frontend/src/pages/MarketData.tsx::CoverageCell` `—` render ediyor. Uydurma span yok.

**Asıl kusur, iddia edilenden farklı ve daha sinsi:** analiz edilmiş bir tick dataset'i ile
**hiç analiz edilmemiş** bir dataset registry'de **ayırt edilemiyordu** — ikisi de kalıcı `—`.
Aynı boşluk `commands/backtest_run_context.py::_data_time_entry`'ye de sızıyordu: tick veriye
bağlanan bir RUN'ın manifestine `coverage: null` pinleniyor, hangi aralığı tükettiğinin kanıtı
kayboluyordu (doc 11 §7.1 "Run / Agent Manifest Reference … coverage slice").

### Seçim: (a) coverage türet — (b) "V1'de yalnız OHLCV" dürüst sınırı REDDEDİLDİ

Kickoff iki seçenek sunuyordu. **(a) seçildi.** Gerekçe:

1. **Spec (b)'yi desteklemiyor.** Doc 11 §3.3 registry tablosunun tick satırı Coverage kolonunu
   **dolu** gösteriyor ("Selected 2025 sessions"). §7.1 coverage'ı `MarketDatasetRevision`'ın
   değişmez teknik gerçeği olarak **tip niteleyicisi olmadan** sayıyor; aynı bölümün
   "Run / Agent Manifest Reference" satırı **coverage slice**'ı da tipten bağımsız pinliyor.
   "Yalnız OHLCV" demek üç ayrı spec satırını birden dürüst sınıra çevirmek olurdu.
2. **Türetilen şey uydurma değil.** Kapsanan aralık = gözlenen ilk/son instant + satır sayısı.
   Cadence gerektirmez; hiçbir şey tahmin edilmez.
3. **Spec'in cadence'e bağlı olan yeri yalnız GAP VERDİKTİ.** §7.4'te "Declared cadence gaps
   recorded" **sadece OHLCV** satırında. Bu yüzden `CADENCE_GAP` uyarısı cadence kapısında kaldı;
   genelleşen tek şey slice üretimi.

### Segmentasyon kuralı: ardışık kapsanan UTC günleri

Tek bir min→max span (b)'den de kötü olurdu: "selected sessions" dataset'i için
`2025-03-03 → 2025-06-10` **kesintisiz kapsama gibi okunur** — P-09'un yasakladığı uydurma span.
Uydurma bir gap eşiği de seçmek istemedik. Bunun yerine sistemin **kendi ilan ettiği** kapsama
granülü kullanıldı: processed asset zaten `instrument + date` partition'ları hâlinde yazılıyor
(doc 11 §7.3 adım 5). Yani:

- `_build_daily_coverage(counts)` — satırı olan **ardışık UTC günleri** tek segment; satırı
  olmayan bir takvim günü segmenti kapatır.
- Kapanan segmentin `gap_seconds`'ı, bir sonraki **gerçekten gözlenen** instant'a olan gerçek
  mesafedir (çıkarsanmış cadence değil).
- **Uyarı üretilmez.** "Selected sessions" dataset'inde eksik gün kusur değil, kasıttır; §7.4'ün
  tick/spread satırları gap'i doğrulama konusu saymıyor. Kayıt tutulur, hüküm verilmez.

### Yan kazanç: cadence'i okunamayan OHLCV

Dal artık `if counts:` ile açılıyor; cadence'i çözülemeyen ya da `resolution_kind != BAR` olan bir
OHLCV revision'ı da (eskiden **sessizce coverage'sız** kalıyordu) günlük segmentasyonla kapsanıyor.

### Değişen dosyalar

| Dosya | Değişiklik |
|---|---|
| `domain/market_data/validation_rules.py` | coverage dalı `if counts:` altına alındı; `_build_daily_coverage` eklendi; `evaluate_cross_row` docstring'i |
| `tests/unit/test_market_cross_row_validation.py` | `coverage == ()` bekleyen 2 test yeniden hizalandı + 2 yeni test (selected-sessions bölünmesi, spread) |
| `tests/integration/test_market_crossrow_validation.py` | `_run` helper'ı tip/kolon parametreli; tick ve spread için gerçek DB'de `dataset_coverage_slice` satır kanıtı |

**Migration YOK** — `dataset_coverage_slice` şeması aynı, yalnız daha önce hiç yazılmayan satırlar
yazılıyor. Alembic head `0039_backtest_run_cancellation`. `ENGINE_VERSION` bump **edilmedi** —
backtest engine'in yürütme semantiği değişmedi.

### Dürüst sınırlar

- **Geriye dönük doldurma yok.** Bu değişiklikten ÖNCE analiz edilmiş tick/spread revision'ları
  slice'sız kalır; registry'de `—` göstermeye devam ederler. Yeniden analiz gerektirir.
- **Frontend'e dokunulmadı.** `CoverageCell` zaten `null` ve dolu digest'i doğru render ediyordu;
  tick satırı artık kendiliğinden dolu geliyor. Registry hücresi `slice_count`'u göstermiyor —
  yani gap'li bir kapsama (OHLCV'de de olduğu gibi) min→max olarak okunuyor. Bu **mevcut** OHLCV
  davranışıyla eşitlik, bu slice'ın getirdiği yeni bir sapma değil; ayrı bir UI kararı.
- **Bitişikte görülen, kapsam dışı bırakılan kusur:** `DUPLICATE_TIMESTAMP` kuralı tüm tiplere
  BLOCKING_FAIL uyguluyor; oysa doc 11 §7.4 tick satırı duplicate kontrolünü "Trade ID/sequence
  available ise" diye koşullandırıyor — aynı milisaniyede iki gerçek trade meşrudur. Bu slice
  o kuralı değiştirmedi.
