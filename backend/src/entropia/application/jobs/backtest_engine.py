...
def _shared_clock_inputs(
    ...
    *,
    ...
    participants: list[ItemParticipant] = []
    shares: dict[str, Decimal] = {}
    # ``pinned``, NOT the name the independent item loop uses. That loop's header is a
    # LITERAL the containment gate greps for to prove it survived the wiring, and a second
    # copy of it anywhere in this module — code OR comment, a text scan cannot tell them
    # apart — lets the gate stay green with the real loop deleted, which is the silent
    # re-price of every independent composite Result. Measured: before this rename, a
    # negative control that removed the real loop left the gate GREEN.
