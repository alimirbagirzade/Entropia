...
**Ne yapıldı.** Traceability'deki `Not started` + "overlaps P-11/12/16" notu bir varsayımdı; bu
slice onu **ölçümle** değiştirdi. Test-dışı tüm `.tsx` üzerinde JSX text-node taraması → 31 dosyada
161 `*_id` render'ı; her biri F-07'nin kendi acceptance'ına göre ("copyable IDs in advanced detail"
serbest, "no common task requires recognizing an opaque identifier" zorunlu) sınıflandırıldı.

**Bulgu.** P-11 (PR #375), P-12 (`3c6887c`), P-16 (`20ccacc`) **gerçekten landed** — traceability'nin
10/13/16 satırlarındaki "raw ids" gerekçeleri bayattı ve düzeltildi (satırlar PO D-4/D-5 + A-06
nedeniyle OPEN kalır). Ama **kalıntı var**: ikisi Portfolio'da (P-11'in süpürdüğü sayfa) presentation-only
düzeltildi; dördü backend display DTO gerektirdiği için açık bırakıldı.

- **Düzeltildi:** `Portfolio.tsx` `IssuesTable` artık `labelByItem` alıp `ItemLabel` render ediyor
  (validation issue'nun işaret ettiği öğe ad ile anılır, id ikincil anahtar); örnek satırının
  `?? composition_item_id` fallback'i `UNLABELLED_ITEM`'a çevrildi (ham id asla birincil ad değil).
  Yeni modül-içi `labelsByCompositionItem()`. Migration yok, ENGINE_VERSION yok, backend'e dokunulmadı.
- **Kabul testi:** `portfolio.test.tsx` "names the item a validation issue points at, instead of a
  bare id (F-07)" — `within(issueRow)` ile etiketin satırın İÇİNDE olduğunu doğrular; **RED kanıtı
  alındı** (render geri alındığında yalnızca bu test kırıldı).
- **Testler:** vitest **608/608** (`--no-file-parallelism`), eslint ✓, tsc ✓.

**Açık kalan dört kalıntı (backend display-DTO slice'ı):** `PreCheck.tsx:124` (request seçici
sütunu), `Library.tsx:1259/1274` (import job satırları), `ResultDetail.tsx:420/589` (per-item
breakdown — immutable manifest, etiket sunucudan gelmeli), `ReadyCheck.tsx:291` (issue `scope_id` —
rapor immutable + `is_current`, bayat raporu canlı kompozisyondan etiketlemek yanlış olur).
Ayrıntı + gerekçe: `docs/implementation/v18_visual_traceability.md §4.4`.
