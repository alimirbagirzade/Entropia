...
### Tier 2 — AYIRILDI, ayrı karar gerektiriyor (2 check)

| Check | Neden required listesinde değil |
|---|---|
| `Lighthouse — score ratchet vs. the seeded stack (RC P11-8)` | **Ölçüm varyansı kabul edilmiş bir kapı.** CLAUDE.md'nin kendi kaydı: `panel-management` tabanı "98'de KALMALI (ölçülmüş 98–100 varyansı; 100'e sıkılaştırmak çırpınan kapı verir)". Tarayıcı skoru runner yüküne bağlı; required yapılırsa ilk çırpınmada insanı "yeniden koştur" refleksine iter ve **bu, tüm kapı setine olan güveni aşındırır**. Ayrıca donmuş kusurları (#677) hâlâ açık. Tavsiye: birkaç hafta gözlem, sıfır kırmızı görülürse §7'deki yolla eklenir. |
...
Payload'daki 15 adı, GitHub'ın o PR'da **gerçekten ürettiği** check listesiyle
...
ölçülen check-run yanıtından üretildi, o yüzden 15 adın tamamı karakteri karakterine
...
| `rules[3]` | `required_status_checks` | 15 check, hepsi `integration_id: 15368` ile **GitHub Actions app'ine sabitli** (ölçüldü). Sabitleme, aynı adı üreten üçüncü bir app'in kapıyı sahte-yeşil geçmesini engeller |
...
8. **Add checks** → arama kutusuna yazın ve **açılan listeden seçin, elle yazıp
   ...
   girişi §5'teki kilidin ta kendisidir. Seçilecek 15 ad §2 Tier 1 tablosunda.
