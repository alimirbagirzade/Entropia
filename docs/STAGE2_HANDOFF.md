...
## T-01 — Testsiz/eksik-kapsanmış kritik modüller kilitlendi (PR #422, merged → main `b54d7d7`)

**Ne landed.** Üç modülün mevcut davranışı teste kilitlendi: `queries/audit_log.py::list_audit_events`
(uyumluluk okuma yolu), `domain/admin_panel/role_matrix.py::build_role_matrix` (politika projeksiyonu),
`queries/allocation_currency.py::resolve_settlement_currencies` (FX cross-check besleyicisi).
**Kaynak koda dokunulmadı** — yeni davranış yok, **migration YOK**, `ENGINE_VERSION` sabit.
3 dosya, +748 satır, **55 test**. Yeni dosyalar: `tests/unit/test_role_matrix_contract.py` (30),
`tests/integration/test_audit_log_read_model.py` (11), `tests/integration/test_allocation_settlement_currency.py` (14).

**Backlog maddesi kısmen yanlıştı — düzeltildi.** `role_matrix` **testsiz değildi**
(`test_admin_panel_taxonomy.py::test_role_matrix_projection` + `test_panel_management_logs.py::test_role_matrix_admin_only`
zaten referans veriyordu); ama 20 rol×yetenek hücresinden **14'ü** pinlenmemişti — endişe doğru,
gerekçe yanlış. `audit_log` ve `allocation_currency` iddiası doğrulandı; ikincisi
`test_allocation_fx_dependency.py` üzerinden **yalnız STRATEGY doğrudan-payload dalını** koşuyordu
(mirror deref + TRADING_SIGNAL + TRADE_LOG dalları hiç çalışmamıştı). Ayrıca brief'in istediği
**audit-log "filtre kombinasyonları" bu modüle ait değil** — `list_audit_events` yalnız
`cursor` + `limit` alır; filtreli yüzey ayrı modüldür (Admin Panel `list_log_events`, doc 19, zaten testli).

**Mutasyon kanıtı.** Üç kaynak modüle kasıtlı birer kusur enjekte edildi, her biri **tam olarak bir**
testi düşürdü, sonra HEAD'e geri alındı: mirror deref devre dışı → mirror testi; `has_more` koruması
düşürüldü → cursor testi; supervisor `view_use` değişti → ilgili hücre testi.

**Review sonucu.** CI 6/6 yeşil (backend 34m14s, frontend, 2×E2E, A11Y, Docker). Lokal: ruff +
format (624 dosya) + mypy (356 dosya) temiz; izole DB'de tam suite **2285 passed, 0 failed, 0 errors, 0 skipped**.

**Ertelenen / kapsam dışı.** Filtreli `list_log_events` kapsam derinliği denetlenmedi. K-09/K-10
extraction'ından doğan yeni engine modüllerinin kapsamı denetlenmedi (T-02 adayı). Frontend'e dokunulmadı.

**İki ortam dersi (ikisi de bu slice'ta ısırdı).** (1) `pytest ... | tail` **kullanma** — exit code
`tail`'in olur; bu slice'ta 20 ERROR veren bir koşu bir tur boyunca "yeşil" sanıldı. (2) Tam suite
koşarken `uv sync`/`uv run` çalıştırma — venv ortadan yeniden kurulur ve sahte ERROR üretir.

Tam kayıt: `docs/PROJECT_HISTORY.md` §"T-01 · Testsiz kritik modüller".
Devam tohumu: `docs/T01_LANDED_KICKOFF.md`.

## Next: **PO imzası + R2 kapanışı** (değişmedi) · test-boşluğu backlog'unda T-02 adayı belirlendi
...
**T-02 adayı:** K-09 (#423/#425/#426/#424) ve K-10 (#428/#429) engine god-module extraction'ından
doğan modüllerin (order matching, stop resolution, position sizing, leverage, cost model, funding
decision, `_Ledger`) test kapsamını **ampirik** denetle. #427'nin golden-digest guard'ı uçtan uca
davranışı pinliyor ama modül-içi dalları pinlemiyor.
