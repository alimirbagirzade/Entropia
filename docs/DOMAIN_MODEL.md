...
- **Last-admin protection:** cannot demote/deactivate the last active Admin. ONE gate (`policy.py::ensure_not_last_admin`), page-specific code bound at the call site: the legacy `PATCH /users/{id}/role` path (Master Module 3) answers `LAST_ADMIN_PROTECTED`; the Panel `PATCH /v1/admin/users/{id}/role` command (Module 16 / doc 19 §7.1, §9.3, §11, §14) answers `LAST_ADMIN_PROTECTION`. Both are 422, and the Panel class subclasses the legacy one so `except LastAdminProtectedError` still catches either. A role change needs no session invalidation and produces no stale-role rejection: `apps/api/deps.py` re-resolves the role from the registry on EVERY request (M1 §4.2), so the next request already runs under the new role. (Master ref §11.1 names a `ROLE_CONTEXT_STALE` code for a session that caches a role revision; `auth_sessions` caches none, so O-03R deleted the class rather than leave an unreachable contract published here.)
