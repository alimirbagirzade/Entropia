...
| GET `/metrics` | `metrics_endpoint` `metrics.py:104` | Prometheus text exposition — **7 aile** (3 process + 4 DB-backed gauge; sonuncusu ADIM 25 `entropia_worker_heartbeat_age_seconds`). Envanter + alert/runbook eşlemesi: `docs/runbooks/METRIC_ALERT_MATRIX.md`. **ADIM 31:** bu exposition'ı scrape eden Prometheus + ateşleyen alarmı bir insana taşıyan Alertmanager `docker-compose.yml`'de `observability` profili altında sevk edilir (düz `docker compose up` onları başlatmaz); yol, silence ve açık artıklar `docs/runbooks/alert-notification.md` | `require_metrics_scraper:42` (O-22 — Bearer `ENTROPIA_METRICS_TOKEN`; yok=401 `METRICS_SCRAPE_UNAUTHORIZED`, yanlış=403 `METRICS_SCRAPE_FORBIDDEN`; token yapılandırılmamışsa production'da fail-closed 403, local'de açık) |
